...
## 프로젝트를 마치며
- Django에 대한 지식 없이 처음 진행했던 프로젝트였기 때문에 기간 초반에는 Django에 대한 학습에 시간을 들여 프로젝트 작업에 많이 참여하지 못하였습니다. 하지만 MTV의 기본 구조 및 Ajax를 이용한 프론트와 Django 서버의 통신 흐름을 이해하고 계속적인 Django 학습을 통해 점차 프로젝트에 기여를 많이 할 수 있게 되었습니다.
- Git을 다루는데 어려움을 겪어 코드를 commit 하거나 merge를 할 때 팀원의 도움을 많이 받았습니다. 프로젝트를 진행하면서 branch 사용 및 PR을 통한 merge전의 코드 리뷰, 작업 단위별 commit을 다루면서 Git을 통한 협업에 대해 익숙해졌습니다.
